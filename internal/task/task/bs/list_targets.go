...
func (s *step2FormatTarget) Execute(ctx *context.Context) error {
	output := *s.output
	if strings.Contains(output, "iscsiadm: No portals found") {
		return nil
	}
	...
	if s.spdk {
		for i, line := range lines {
			tgt := strings.Split(line, " ")
			if len(tgt) < 2 {
				return errno.ERR_DISCOVER_OUTPUT_FORAMT_FAILED
			}
			target2volume := strings.Split(tgt[1], ":")
			if len(target2volume) < 2 {
				return errno.ERR_DISCOVER_OUTPUT_FORAMT_FAILED
			}
			store := "/" + strings.ReplaceAll(target2volume[1], "-", "/")
			target = &Target{
				Host:   s.host,
